...
type LoginMode = 'default' | 'nafath-id' | 'nafath-waiting';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleNafathSubmit = () => {
    if (!nationalId) {
      setNationalIdError('يرجى إدخال رقم هويتك الوطنية');
      return;
    }
    if (nationalId.length !== 10 || !/^\d+$/.test(nationalId)) {
      setNationalIdError('يجب أن يكون رقم الهوية 10 أرقام');
      return;
    }
    const num = Math.floor(10 + Math.random() * 90).toString();
    setRandomNumber(num);
    setCountdown(5);
    setLoginMode('nafath-waiting');
  };
    ...
    <div className="login-container">
      ...
      <div className="login-form-section">
        <div className="form-wrapper">
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="login-form">
                ...
                <div className="form-footer">
                  ...
                  <a href="#forgot" className="forgot-password">هل نسيت كلمة المرور؟</a>
                </div>
              ...
              <button className="nafath-button" onClick={() => setLoginMode('nafath-id')}>
              ...
              <div className="signup-link">
                <p>ليس لديك حساب؟{' '}
                  <a href="#signup" className="signup-text">إنشاء حساب جديد</a>
                </p>
              </div>
            </>
          )}

          {/* Nafath: national ID input */}
          {loginMode === 'nafath-id' && (
            <>
              <div className="form-header">
                <h3>تسجيل الدخول بنفاذ</h3>
                <p>أدخل رقم هويتك الوطنية للمتابعة</p>
              </div>

              <div className="nafath-form">
                <div className="form-group">
                  <label className="form-label">رقم الهوية الوطنية</label>
                  <input
                    type="text" maxLength={10}
                    value={nationalId}
                    onChange={e => { setNationalId(e.target.value); setNationalIdError(''); }}
                    placeholder="أدخل رقم الهوية (10 أرقام)"
                    className={`form-input ${nationalIdError ? 'input-error' : ''}`}
                    dir="ltr"
                  />
                  {nationalIdError && <span className="error-message">{nationalIdError}</span>}
                </div>

                <button className="submit-button" onClick={handleNafathSubmit}>
                  متابعة
                </button>

                <button className="back-link" onClick={() => { setLoginMode('default'); setNationalId(''); setNationalIdError(''); }}>
          ...
          {/* Nafath: waiting with code and timer */}
          {loginMode === 'nafath-waiting' && (
            <div className="nafath-waiting">
              <div className="nafath-waiting-icon">
                <svg width="48" height="48" viewBox="0 0 765 83" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <h3 className="nafath-waiting-title">في انتظار الموافقة</h3>
              <p className="nafath-waiting-desc">افتح تطبيق نفاذ واختر الرقم التالي</p>
              <div className="nafath-code">{randomNumber}</div>
              <div className="nafath-timer">
                <div className="nafath-timer-bar" style={{ width: `${(countdown / 5) * 100}%` }}></div>
              </div>
              <p className="nafath-timer-text">جاري تسجيل الدخول... ({countdown})</p>
            </div>
          )}
        </div>
      </div>
